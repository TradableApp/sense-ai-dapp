...
import ManageLimitDialog from './ManageLimitDialog';
...
function formatTimeRemaining(expiryDate) {
	...
	if (daysRemaining <= 0) return 'Limit has expired';
...
export default function PlanStatusCard({ plan }) {
	...
	const spentPercentage = (spentAmount / allowance) * 100;
	...
	return (
		<Card className="mb-8 w-full max-w-md rounded-2xl border-border/30 bg-background shadow-lg">
			...
			<CardContent className="space-y-4">
				...
				<Separator />
				{/* UPDATED: This container now stacks vertically on mobile and horizontally on larger screens. */}
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<div className="w-full">
								<Button className="h-10 w-full text-base font-medium" disabled={hasPendingPrompts}>

		...
		return (
			...
			>
				<div
					ref={ref}
					className={cn(
						'flex h-full w-full has-data-[variant=inset]:bg-muted',
						className,
					)}
					style={
						{
...
const SidebarInset = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
	<main ref={ref} className={cn('flex h-full flex-1 flex-col overflow-hidden', className)} {...props} />
));
...
const SidebarMenuAction = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement> & { asChild?: boolean }>(
...
const SidebarGroupLabel = React.forwardRef<HTMLSpanElement, React.HTMLAttributes<HTMLSpanElement>>(({ className, ...props }, ref) => (
	<span
		ref={ref}
		className={cn(
